...
def scRNA2PseudoBulkAnnData(adata, sample_id_col = None): 
    ...
    for sample in adata.obs['sample_id'].unique():
        ...
        mask = (adata.obs['sample_id'] == sample).values  # Convert to numpy array
        tpm = np.sum(adata.X[mask, :], axis = 0)
...
def calculate_gene_correlation(results, adata_1_pseudo, adata_2_pseudo, method='pearson'):
    ...
    for index, row in results.iterrows():
        ...
        # Check if genes exist in var_names, if not, replace '.' with '-'
        if g1 not in adata_1_pseudo.var_names:
            g1 = g1.replace('.', '-')
        if g2 not in adata_2_pseudo.var_names:
            g2 = g2.replace('.', '-')
        
        # Continue only if both g1 and g2 are found after modification
